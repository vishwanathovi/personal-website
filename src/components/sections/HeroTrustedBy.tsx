...
const CLIENTS = [
  { name: 'Microsoft', path: PATH_MICROSOFT },
  { name: 'Apple', path: PATH_APPLE },
  { name: 'ByteDance', path: PATH_BYTEDANCE },
  { name: 'OpenAI', path: PATH_OPENAI },
  { name: 'Meta', path: PATH_META },
] as const
      ...
      <ul className="flex flex-wrap items-start gap-x-5 gap-y-4 sm:gap-x-7 sm:gap-y-5 lg:gap-x-6 lg:gap-y-4">
        {CLIENTS.map(({ name, path }) => (
          <li key={name} className="flex w-[4.5rem] flex-col items-center gap-1.5 text-center sm:w-[5.25rem]">
            <svg
              viewBox="0 0 24 24"
              className="h-5 w-auto max-w-full shrink-0 text-stone-400 opacity-90 transition-opacity hover:text-stone-300 sm:h-6"
              fill="currentColor"
              aria-hidden
            >
              <path d={path} />
            </svg>
            <span className="w-full text-[10px] font-medium leading-tight text-stone-500 sm:text-[11px]">
              {name}
            </span>
          </li>
        ))}
