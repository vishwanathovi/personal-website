...
export function ScrollyLayout({ config, configId, children }: ScrollyLayoutProps) {
  ...
  useEffect(() => {
    ...
    const prevHtmlDark = document.documentElement.classList.contains('dark')
    const prevHtmlLight = document.documentElement.classList.contains('light')
    ...
    return () => {
      ...
      document.documentElement.classList.toggle('dark', prevHtmlDark)
      document.documentElement.classList.toggle('light', prevHtmlLight)
    }
  }, [configId, page.theme])
